...
По умолчанию проверяются 50 групп. Доступны `--limit=100`, `--seed=20260830`,
`--all` и `--group=КТ-41-24` (аргументы передаются после `--`). Значения
авторизации скрипт не выводит.

### Корпус сырых фикстур расписания

Собрать обновляемый корпус из 50 случайных групп, включая `КТ-41-24`:

```bash
pnpm fixtures:collect -- --limit=50 --periods=1,2,3,4 --seed=20260830
```

Утилита лежит в `utils/collectScheduleFixtures.mjs` и сохраняет только сырые
HTML-таблицы в `test/fixtures/tt/group-schedules/`. Группа считается выбранной
только если в исходной разметке есть занятия. Манифеста, построенного
парсером, нет: тест `test/tt-fixtures.test.mjs` сам перечисляет файлы,
проверяет исходную разметку и сравнивает её с результатом парсинга. Отдельный
JSON-файл содержит только вручную проверенные инварианты для `КТ-41-24`.

После сбора запустить корпус можно обычной командой:

```bash
node --test test/tt-fixtures.test.mjs
```
