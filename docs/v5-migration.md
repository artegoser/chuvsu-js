...
Начиная с 5.2.0 поиск совпадающих занятий, справочник сущностей и удаление пустых
канонических записей используют локальные индексы. Одинаковые повторные снимки не
меняют ревизию. `compareAndSetPatch` получает только `directoryUpserts` и
`sourceReplacements`, затронутые одной мутацией. Поэтому размер записи и обычное
время обновления не растут вместе со всем репозиторием. Старый `compareAndSet`
поддерживается как fallback, но сериализует полный снимок.

`exportRepository()` по-прежнему имеет цену, пропорциональную размеру полного
