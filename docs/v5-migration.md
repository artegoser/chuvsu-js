...
Начиная с 5.1.2 поиск совпадающих занятий и выборка расписания владельца используют
внутренние индексы, одинаковые повторные снимки не меняют ревизию, а периоды одного
запроса сохраняются одной транзакцией адаптера. Это исключает полный обход
репозитория из обычного пути запроса; `exportRepository()` по-прежнему имеет цену,
пропорциональную размеру полного снимка, и не должен вызываться на клиентском пути.
