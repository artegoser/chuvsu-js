...
> [!WARNING]
> Ооочень плохой вайбкод, библиотека будет переделана и не раз. Есть очень много пространства для упрощения использования и сохранения крутой архитектуры.
...
Переход с версии 4: [`docs/v5-migration.md`](docs/v5-migration.md).
...
процессы не перезаписывали установленные ID. Для БД реализуйте
`compareAndSetPatch`: обычное обновление передает только измененные сущности и
полные замены затронутых источников. `compareAndSet` с полным снимком оставлен
как совместимый fallback; его не следует использовать на горячем пути большого
репозитория.
