...
использует compare-and-set по ревизии. Предпочтительный метод
`compareAndSetPatch` атомарно применяет только измененные строки справочника и
полностью заменяет затронутые источники со связями. Обычная мутация не экспортирует
и не обходит весь репозиторий; полный снимок остается форматом загрузки, переноса и
